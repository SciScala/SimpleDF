...
import org.sciscala.simpledf.arrayseq.ArraySeqDataFrameReader.arrayseqDataFrameReader
...
yellowbeard,16,17""".stripMargin
    ...
    arrayseqDataFrameReader.readCSV(
      csv,
      schema,
      true,
      Some("name")
...
yellowbeard,16,17""".stripMargin
  ...
  "DataFrameReader" should "read a Json with a simple Object from a String" in {
    ...
    arrayseqDataFrameReader.readJson(simpleObj, None).data.head.head shouldBe jsonData.head.head
  }
  ...
  "DataFrameReader" should "read a Json with a simple Array from a String" in {
    ...
    val parsed = arrayseqDataFrameReader.readJson(simpleArr, None)
  ...
  "DataFrameReader" should "read a Json with a nested Object from a String" in {
    ...
    val parsed = arrayseqDataFrameReader.readJson(dataString, None)

    println(parsed.data)
    println(parsed.index)
    println(parsed.columns)

    parsed.data.head shouldBe quotes
  }

  "DataFrameReader" should "read a Json with a simple Array from a file" in {
    val parsed = arrayseqDataFrameReader.readJson(Paths.get(getClass.getResource("/test.json").getPath), None)
  ...
  "DataFrameReader" should "read a Json with a nested Object from a file" in {
    ...
    val parsed = arrayseqDataFrameReader.readJson(Paths.get(getClass.getResource("/USD.json").getPath), None).data.head
