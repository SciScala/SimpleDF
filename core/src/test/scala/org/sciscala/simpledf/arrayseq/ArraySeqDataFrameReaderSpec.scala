...
yellowbeard,16,17""".stripMargin
  ...
  "DataFrameReader" should "read a Json with a simple Object from a String" in {
    ...
    ArraySeqDataFrameReader.arrayseqDataFrameReader.readJson(simpleObj, None).data.head.head shouldBe jsonData.head.head
  }
  ...
  "DataFrameReader" should "read a Json with a simple Array from a String" in {
    ...
    val parsed = ArraySeqDataFrameReader.arrayseqDataFrameReader.readJson(simpleArr, None)
  ...
  "DataFrameReader" should "read a Json with a nested object from a String" in {
    ...
    val parsed = ArraySeqDataFrameReader.arrayseqDataFrameReader.readJson(dataString, None).data.head

    parsed shouldBe quotes
  }

  "DataFrameReader" should "read a Json with a simple Array from a file" in {
    val parsed = ArraySeqDataFrameReader.arrayseqDataFrameReader.readJson(Paths.get(getClass.getResource("/test.json").getPath), None)
  ...
  "DataFrameReader" should "read a Json with a nested object from a file" in {
    ...
    val parsed = ArraySeqDataFrameReader.arrayseqDataFrameReader.readJson(Paths.get(getClass.getResource("/USD.json").getPath), None).data.head
